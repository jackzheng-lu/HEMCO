...
MODULE HCOX_GC_RnPbBe_Mod
    ...
    !
      TYPE :: MyInst
       ...
       INTEGER               :: ExtNrZheng    ! ZHANG_Rn222 extension number
       ...
       ! >>> mzheng
       INTEGER               :: IDTBe10Trop90N ! Index # for Be10 60-90 N troposphere
       INTEGER               :: IDTBe10Trop60N ! Index # for Be10 30-60 N troposphere
       INTEGER               :: IDTBe10Trop30N ! Index # for Be10 00-30 N troposphere
       ...
       ! >>> mzheng
       REAL(hp), POINTER     :: EmissBe10Trop90N(:,:,:)
       REAL(hp), POINTER     :: EmissBe10Trop60N(:,:,:)
       REAL(hp), POINTER     :: EmissBe10Trop30N(:,:,:)
    ...
    !
         ...
         ! >>>mzheng    11/26/2021 - M.Zheng Add switch for Be10 emission options, mzheng
        INTEGER            :: IU_FILE_be7, IU_FILE_be10, IOS_be7, IOS_be10 ! Unit for emis file
        INTEGER            :: Be_EMIS_MONTH ! Read emis for this month
        ! noted!! 
        ! > found that setting a fixed array will require large space causing segmental fault
        ! using allocatable
        !  REAL*8, dimension(72,46,72)  :: SW_BE7_ARR, SW_BE10_ARR ! Readin Be emis array lon, lat, lev    
        ! >>> mzheng
    ! !REMARKS:
    ...
    !
        ...
        IF ( Inst%IDTBe7 > 0 .or. Inst%IDTBe10 > 0 ) THEN
           ...
           IF ( Inst%ExtNrZheng > 0 ) THEN
    
              !------------------------------------------------------------------
              ! Use Zhang et al Rn222 emissions
              ! cf https://doi.org/10.5194/acp-21-1861-2021
              !------------------------------------------------------------------
              CALL HCO_EvalFld( HcoState,       'ZHENG_BE10_EMIS',              &
                                Inst%EmissBe10, RC                             )
              IF ( RC /= HCO_SUCCESS ) THEN
                 CALL HCO_Error( 'Could not read ZHENG_BE10_EMIS!', RC )
                 RETURN
              ENDIF
              CALL HCO_EvalFld( HcoState,       'ZHENG_BE7_EMIS',              &
                                Inst%EmissBe7, RC                             )
              IF ( RC /= HCO_SUCCESS ) THEN
                 CALL HCO_Error( 'Could not read ZHENG_BE7_EMIS!', RC )
                 RETURN
              ENDIF
              WRITE(*,*) 'OPEN it sccessfully'
            ...
            !$OMP SCHEDULE( DYNAMIC )
               ...
               DO I = 1, HcoState%Nx
                       ...
                       ! the Beryllium emissions [atom Beryllium/g air/s] *air (kg)* g/kg / [atoms/kg]/area(m2)
                       ! so kg/m2/s
                         ADD_Be7     = Inst%EmissBe7 (I,J,L)* ExtState%AIR%Arr%Val(I,J,L) * 1.e+3_hp / XNUMOL_Be7/ HcoState%Grid%AREA_M2%Val(I,J)
                         ADD_Be10     = Inst%EmissBe10 (I,J,L)* ExtState%AIR%Arr%Val(I,J,L) * 1.e+3_hp / XNUMOL_Be10/ HcoState%Grid%AREA_M2%Val(I,J)
             
                      ! Save emissions into an array for use below  [kg/m2/s]
                       Inst%EmissBe7 (I,J,L) = ADD_Be7
                       Inst%EmissBe10(I,J,L) = ADD_Be10
             
                       IF (ADD_Be10 < 0 .or. ADD_Be7 < 0) THEN
                       ...
                       ELSE
                          ...
                          ENDIF
                          IF ( Inst%IDTBe10Strat > 0 ) THEN
                             Inst%EmissBe10Strat(I,J,L) = 0_hp
                          ENDIF
         ...
         !  ! >>> mzheng, this part of original code is wrong;(backup)
         !  ! > For Be10 it should be [0.025 atoms/start], see Lal 1967 table
         ...
         !  ADD_Be10 = ( Be_TMP / XNUMOL_Be10 ) / HcoState%Grid%AREA_M2%Val(I,J) ! mzheng, here is the place wrong

         !  ! <<< mzheng, this part of original code is wrong (backup)
         

          ! >>> mzheng, corrected the Be10 yield function base on Lal 1967 Table
          ! Be_TMP = [stars/g air/s] * [0.045 atom/star] *
          !          [kg air] * [1e3 g/kg] = 7Be/10Be emissions [atoms/s]
               Be_TMP  = Be_TMP * ExtState%AIR%Arr%Val(I,J,L) * 1.e+3_hp

               ! ADD_Be = [atoms/s] / [atom/kg] / [m2] = 7Be/10Be emissions [kg/m2/s]
               ADD_Be7  = ( Be_TMP* 0.045e+0_hp  / XNUMOL_Be7  ) / HcoState%Grid%AREA_M2%Val(I,J)
               ADD_Be10 = ( Be_TMP* 0.025e+0_hp  / XNUMOL_Be10 ) / HcoState%Grid%AREA_M2%Val(I,J) ! mzheng, here is the place wrong
               ! <<< mzheng, corrected the Be10 yield function base on Lal 1967 Table
              ...
              ! mzheng, note default is zero, so no need to put zero into input again
              IF ( L > ExtState%TropLev%Arr%Val(I,J) ) THEN
                 IF ( Inst%IDTBe7Strat > 0 ) THEN
                    Inst%EmissBe7Strat (I,J,L) = Add_Be7
                 ENDIF
    ...
    !$OMP END PARALLEL DO
           ...
           ! >>> mzheng
           IF ( Inst%IDTBe10Trop90N > 0 ) THEN
              Arr3D => Inst%EmissBe10Trop90N(:,:,:)
              CALL HCO_EmisAdd( HcoState, Arr3D, Inst%IDTBe10Trop90N, &
                                RC,       ExtNr=Inst%ExtNr )
              Arr3D => NULL()
              IF ( RC /= HCO_SUCCESS ) THEN
                 CALL HCO_ERROR( &
                                 'HCO_EmisAdd error: EmissBe10Trop90N', RC )
                 RETURN
              ENDIF
           ENDIF

           IF ( Inst%IDTBe10Trop60N > 0 ) THEN
              Arr3D => Inst%EmissBe10Trop60N(:,:,:)
              CALL HCO_EmisAdd( HcoState, Arr3D, Inst%IDTBe10Trop60N, &
                                RC,       ExtNr=Inst%ExtNr )
              Arr3D => NULL()
              IF ( RC /= HCO_SUCCESS ) THEN
                 CALL HCO_ERROR( &
                                 'HCO_EmisAdd error: EmissBe10Trop60N', RC )
                 RETURN
              ENDIF
           ENDIF

           IF ( Inst%IDTBe10Trop30N > 0 ) THEN
              Arr3D => Inst%EmissBe10Trop30N(:,:,:)
              CALL HCO_EmisAdd( HcoState, Arr3D, Inst%IDTBe10Trop30N, &
                                RC,       ExtNr=Inst%ExtNr )
              Arr3D => NULL()
              IF ( RC /= HCO_SUCCESS ) THEN
                 CALL HCO_ERROR( &
                                 'HCO_EmisAdd error: EmissBe10Trop30N', RC )
                 RETURN
              ENDIF
           ENDIF

           IF ( Inst%IDTBe10Trop30S > 0 ) THEN
              Arr3D => Inst%EmissBe10Trop30S(:,:,:)
              CALL HCO_EmisAdd( HcoState, Arr3D, Inst%IDTBe10Trop30S, &
                                RC,       ExtNr=Inst%ExtNr )
              Arr3D => NULL()
              IF ( RC /= HCO_SUCCESS ) THEN
                 CALL HCO_ERROR( &
                                 'HCO_EmisAdd error: EmissBe10Trop30S', RC )
                 RETURN
              ENDIF
           ENDIF

           IF ( Inst%IDTBe10Trop60S > 0 ) THEN
              Arr3D => Inst%EmissBe10Trop60S(:,:,:)
              CALL HCO_EmisAdd( HcoState, Arr3D, Inst%IDTBe10Trop60S, &
                                RC,       ExtNr=Inst%ExtNr )
              Arr3D => NULL()
              IF ( RC /= HCO_SUCCESS ) THEN
                 CALL HCO_ERROR( &
                                 'HCO_EmisAdd error: EmissBe10Trop60S', RC )
                 RETURN
              ENDIF
           ENDIF

           IF ( Inst%IDTBe10Trop90S > 0 ) THEN
              Arr3D => Inst%EmissBe10Trop90S(:,:,:)
              CALL HCO_EmisAdd( HcoState, Arr3D, Inst%IDTBe10Trop90S, &
                                RC,       ExtNr=Inst%ExtNr )
              Arr3D => NULL()
              IF ( RC /= HCO_SUCCESS ) THEN
                 CALL HCO_ERROR( &
                                 'HCO_EmisAdd error: EmissBe10Trop90S', RC )
                 RETURN
              ENDIF
           ENDIF
    ...
    !
        ...
        ! >>> mzheng
        ExtNrZheng = GetExtNr( HcoState%Config%ExtList, 'ZHENG_BE7BE10' )
        ! <<<mzheng
        ...
        DO N = 1, nSpc
           SELECT CASE( TRIM( SpcNames(N) ) )
              ...
              ! >>>mzheng
              CASE( 'Be10Trop90N', '10BeTrop90N' )
                 Inst%IDTBe10Trop90N = HcoIDs(N)  
              CASE( 'Be10Trop60N', '10BeTrop60N' )
                 Inst%IDTBe10Trop60N = HcoIDs(N) 
              CASE( 'Be10Trop30N', '10BeTrop30N' )
                 Inst%IDTBe10Trop30N = HcoIDs(N) 
              CASE( 'Be10Trop90S', '10BeTrop90S' )
                 Inst%IDTBe10Trop90S = HcoIDs(N)  
        ...
        ENDIF
         ...
         ! >>> mzheng 
         IF ( Inst%IDTBe10Trop90N > 0 ) THEN
           ALLOCATE( Inst%EmissBe10Trop90N( HcoState%Nx, HcoState%NY, HcoState%NZ ), &
                     STAT=RC )
           IF ( RC /= 0 ) THEN
              CALL HCO_ERROR ( &
        ...
        IF ( Inst%IDTBe10Trop60N > 0 ) THEN
           ALLOCATE( Inst%EmissBe10Trop60N( HcoState%Nx, HcoState%NY, HcoState%NZ ), &
                     STAT=RC )
           IF ( RC /= 0 ) THEN
              CALL HCO_ERROR ( &
        ...
        IF ( Inst%IDTBe10Trop30N > 0 ) THEN
           ALLOCATE( Inst%EmissBe10Trop30N( HcoState%Nx, HcoState%NY, HcoState%NZ ), &
                     STAT=RC )
           IF ( RC /= 0 ) THEN
              CALL HCO_ERROR ( &
        ...
        ENDIF
         ...
         IF ( Inst%IDTBe10Trop90S > 0 ) THEN
           ALLOCATE( Inst%EmissBe10Trop90S( HcoState%Nx, HcoState%NY, HcoState%NZ ), &
                     STAT=RC )
           IF ( RC /= 0 ) THEN
              CALL HCO_ERROR ( &
        ...
        IF ( Inst%IDTBe10Trop60S > 0 ) THEN
           ALLOCATE( Inst%EmissBe10Trop60S( HcoState%Nx, HcoState%NY, HcoState%NZ ), &
                     STAT=RC )
           IF ( RC /= 0 ) THEN
              CALL HCO_ERROR ( &
        ...
        IF ( Inst%IDTBe10Trop30S > 0 ) THEN
           ALLOCATE( Inst%EmissBe10Trop30S( HcoState%Nx, HcoState%NY, HcoState%NZ ), &
                     STAT=RC )
           IF ( RC /= 0 ) THEN
              CALL HCO_ERROR ( &
    ...
    !
        ...
        ELSE IF (U.LE.X(2)) THEN
           IP=1
        ELSE IF (U.GE.X(N-1)) THEN ! 这里有点问题，IP应该为N-1吧?
           IP=N-2
        ELSE
           I=1
           J=N
    10     IF (IABS(I-J).NE.1) THEN  ! 找到合适的I,J，做到X(I)<U<X(J)
              L=(I+J)/2
              IF (U.LT.X(L)) THEN ! U < X(L)
                 J=L
              ELSE
                 I=L
              END IF
              GOTO 10
           END IF
           IF (ABS(U-X(I)).LT.ABS(U-X(J))) THEN !  U
              IP=I-1
           ELSE
              IP=I
           END IF
        ...
        IF ( ASSOCIATED(Inst) ) THEN
           ...
           IF ( ASSOCIATED(PrevInst) ) THEN
              ...
              IF ( ASSOCIATED(Inst%BESOU         ) ) DEALLOCATE(Inst%BESOU         )
               ! >>> mzheng
              IF ( ASSOCIATED(Inst%EmissBe10Trop30N) ) DEALLOCATE(Inst%EmissBe10Trop30N)
              IF ( ASSOCIATED(Inst%EmissBe10Trop60N) ) DEALLOCATE(Inst%EmissBe10Trop60N)
              IF ( ASSOCIATED(Inst%EmissBe10Trop90N) ) DEALLOCATE(Inst%EmissBe10Trop90N)
